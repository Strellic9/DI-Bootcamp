console.log('_____exc2______')

// var form0 = document.forms["names"];
// console.log(form0);

// // var input0 = document.getElementById('fname');
// // var input1 = document.getElementById('lname');

// var input0 = document.getElementsByName('fname')[0];
// var input1 = document.getElementsByName('lname')[0];
// console.log(input0, input1);

// var ul = document.getElementById('output');

// function addLi(input) {
//     var li = document.createElement("li");
//     li.innerText = input.value;
//     ul.append(li);
// }

// form0.addEventListener('submit', function(event) {
//    event.preventDefault();
//    if (input0.value!=='') { addLi(input0);  }
//    if (input1.value!=='') { addLi(input1);  }
//    console.log(ul);
// })

console.log('_____exc3______');

var allBoldItems=[null];
var bolds=document.getElementsByTagName('strong');
console.log(bolds);

function getBoldItems() {
    for (let i=0; i<bolds.length; i++) {
        console.log(i)
        console.log(bolds[i].textContent);
        if (i==0) { allBoldItems+=bolds[i].textContent;}
        else {allBoldItems+=','+bolds[i].textContent};
    }
    allBoldItems=allBoldItems.split(',')
    console.log(allBoldItems);
}
getBoldItems();

// for (let i=0; i<bolds.length; i++) {
//     bolds[i].addEventListener ("mouseover", function highlight() {
//     bolds[i].style.color='blue';
// })};

// for (let i=0; i<bolds.length; i++) {
//     bolds[i].addEventListener ("mouseout", function return_normal() {
//     bolds[i].style.color='black';
// })};

function highlight() {
    for (let i=0; i<bolds.length; i++) {
        bolds[i].style.color='blue';
    }
}
// highlight();

function return_normal() {
    for (let i=0; i<bolds.length; i++) {
        bolds[i].style.color='black';
    }
}
// return_normal();

let ps = document.getElementsByTagName('p');
ps[0].addEventListener ("mouseover", highlight);
ps[0].addEventListener ('mouseout', return_normal);


